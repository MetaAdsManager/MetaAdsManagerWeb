...
import { autobind, merge } from '@MetaAdsManager/decorator';
...
export default class AliYunWeb {
    ...
    async upload(fileList: File[], options?: IOptions): Promise<string[]> {
        ...
        async function uploadFile({ file, index }: { file: File; index: number }) {
            ...
            return content.cdnDomain + '/' + fileName;
        }

        return Promise.all<string>(fileList.map((file, index) => uploadFile({ file, index })));
    }
