...
import { StatisticsOrderDetailsOutputDto } from '@MetaAdsManager/backend-api/src/request/data-contracts'
...
import { MOrderChannelType } from '@MetaAdsManager/shop-data/src/enums/order/EOrderStatus'
import PayableAmount from '../../../../orderManagement/orderList/components/payableAmount'

export interface IOrderDetailsProps {
  params: Record<string, any>

  onOrderDetail(data: StatisticsOrderDetailsOutputDto): void
}

const Component: FC<IOrderDetailsProps> = (props) => {
  const [loading, setLoading] = useState(false)
  const [columns] = useState<ColumnsType<StatisticsOrderDetailsOutputDto>>([
  ...
  const pageSize = 20
  const [dataSource, setDataSource] = useState<StatisticsOrderDetailsOutputDto[]>([])
