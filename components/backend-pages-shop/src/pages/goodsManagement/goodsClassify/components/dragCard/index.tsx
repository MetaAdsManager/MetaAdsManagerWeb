        ...
        <>
            <Row className={styles.collapseItem}>
                <Col span={16}>
                    {isLastLevel || !hasChildren ? (
                        <Space>
                            ...
                            {!!item.origin.pic && (
                                <Image
                                    src={item.origin.pic + getResizeUrl(iconStyle)}
                                    style={iconStyle}
                                    preview={{ src: item.origin.pic }}
                                />
                            )}
                        </Space>
                    ) : (
                        <span
                            className={styles.collapseItemExpend}
                ...
                <Col span={8} className={styles.operation}>
                    <Space>
                        <Switch
                            checked={item.origin.frontShow}
                            disabled
                            checkedChildren='显示'
                            unCheckedChildren='隐藏'
                        />
                        {/* {props.onPush && (
                            <Button {...buttonProps} onClick={() => props.onPush(item)}>
                                新增分组
                            </Button>
                        )} */}
...
const DragCard: FC<IDragCardProps> = (props) => {
    const { list, droppableId, level = 1, onPush, ...itemProps } = props;
            ...
            {(provided) => (
                ...
                >
                    {list.map((item, index) => {
                        ...
                        const useDraggedId =
                            droppableId + '-' + item.key + '-' + JSON.stringify(rest);
