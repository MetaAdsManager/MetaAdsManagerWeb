...
import flatTree from '@MetaAdsManager/utils/src/tree/flatTree';
import { ICoventTree } from '@MetaAdsManager/utils/src/tree/types';
import sliceTree from '@MetaAdsManager/utils/src/tree/sliceTree';
import mmFormRule, { concatRule } from '@MetaAdsManager/form-rules';
import cc from '../../../config';
import { MenuTreeOutputDto } from '@MetaAdsManager/backend-api/src/request/data-contracts';
import ProFormLimitInput from '@MetaAdsManager/backend-pro/src/components/form/proFormLimitInput';
import MaterialSelect from '@MetaAdsManager/backend-pages/src/components/material/components/materialSelect';
import { MaterialType } from '@MetaAdsManager/backend-pages/src/components/material/const';
...
const Component: FC<IMMGoodsClassifyPageProps> = (props) => {
    ...
    return (
        <PageContainer>
            ...
            <ModalForm
                {...modalProps}
                initialValues={{ pid: 0, frontShow: true }}
            ...
            >
                ...
                <ProFormLimitInput
                    ...
                    fieldProps={{}}
                />
                <ProFormField label='分类图片' name='pic'>
                ...
                {/* <ProFormLimitInput
                    ...
                    fieldProps={{}}
                /> */}
                <ProFormSwitch label='是否显示' name='frontShow' />
            </ModalForm>
        </PageContainer>
    );
};
...
export function useGoodClassifyService(option: IMMGoodsClassifyPageProps) {
    ...
    const slectTree = useMemo(() => {
        ...
        return pLevel === 0
            ? editData
                ? [{ label: '无', value: 0 }]
                : [{ label: '无', value: 0 }].concat(
                      sliceTree(treeList, config.maxClassifyLevel - 1)
                  )
    ...
    const handleSort = useCallback(
        async (result: DropResult) => {
            ...
            await option.api.move({
                id: sourceData.key as any,
                sort: destination?.index,
                pid: sourceData.parentValue as any
            });
    ...
    const handleFormFinish = async (data) => {
        try {
            ...
            const saveData = { ...eData, ...rest, level: data.pid === 0 ? 1 : 2 };
            if (eData) {
                await option.api.update(saveData);
            } else {
                await option.api.add({ ...saveData, sort: 1 });
            }
