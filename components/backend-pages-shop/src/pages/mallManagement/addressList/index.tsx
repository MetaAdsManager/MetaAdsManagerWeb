...
import ProFormCityCascader from '@MetaAdsManager/backend-pro/src/components/form/proFormCityCascader';
import getCityInfoById from '@MetaAdsManager/backend-pro/src/utils/getCityInfoByIds';
import ProFormLimitInput from '@MetaAdsManager/backend-pro/src/components/form/proFormLimitInput';
import { AddressOutputDto } from '@MetaAdsManager/backend-api/src/request/data-contracts';
...
const Component: FC<IAddressListProps> = (props) => {
    const [columns] = useState<ProColumns<AddressOutputDto>[]>([
        ...
        { title: '电话', dataIndex: 'mobile' },
        {
            ...
            render: (_v, { provinceName = '', cityName = '', areaName = '', singleAddress = '' }) =>
                [provinceName, cityName, areaName, singleAddress].join(' ')
        },
        {
            title: '默认设置',
            dataIndex: 'isDefault',
                ...
                return (
                    <>
                        {isSend && (
                            <Tooltip
                                title={
                                    isSend ? '当前地址为默认发货地址，发货时将默认选中此地址' : null
                                }
                            >
    ...
    const { request, actionRef } = useProTableRequest<AddressOutputDto, any>(
        async (params: any) => {
            const { data: list = [], ...rest } = await api['/admin/mall/address/queryListAll_GET'](
                params
            );
            return {
                ...rest,
    ...
    const [handleFormFinish] = useSuperLock(async (values) => {
        try {
            ...
            const { province, city, district } = getCityInfoById({
                province: provinceId,
                city: cityId,
                district: areaId
            });
