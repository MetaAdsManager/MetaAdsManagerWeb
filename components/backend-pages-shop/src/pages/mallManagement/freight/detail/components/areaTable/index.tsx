...
function bindTableKey(
    item: IFormItemAreaValues,
    index: number
): IFormItemAreaValues & { __key__?: string } {
...
const Component: FC<IFormTableProps> = (props) => {
    ...
    const columns: ColumnsType<IFormItemAreaValues> = useMemo(() => {
        const columnsData = [
            ...
            },
            {
                title: (
                    <span className={styles.required}>
                        {valuationFirstName[valuationType] || '-'}
                    </span>
                ),
                dataIndex: 'first',
                render(_va, _re, index: number) {
                    return (
                        <ProFormDigit
                            name={['expressTemplateAreas', index, 'first']}
                            rules={mmFormRule.required}
                            fieldProps={intgerProps as any}
                        />
                    );
                }
            },
            {
                ...
                render(_va, _re, index: number) {
                    return (
                        <ProFormDigit
                            name={['expressTemplateAreas', index, 'firstPrice']}
                            rules={mmFormRule.required}
                            fieldProps={amountProps as any}
                        />
                    );
                }
            },
            {
                title: (
                    <span className={styles.required}>
                        {valuationSecondName[valuationType] || '-'}
                    </span>
                ),
                dataIndex: 'plus',
                render(_va, _re, index: number) {
                    return (
                        <ProFormDigit
                            name={['expressTemplateAreas', index, 'plus']}
                            rules={mmFormRule.required}
                            fieldProps={intgerProps as any}
                        />
                    );
            ...
            },
            {
                ...
                render(_va, _re, index: number) {
                    return (
                        <ProFormDigit
                            name={['expressTemplateAreas', index, 'plusPrice']}
                            rules={mmFormRule.required}
                            fieldProps={amountProps as any}
                        />
                    );
                    ...
                    return (
                        <Space>
                            <Button
                                size='small'
                                type='link'
                                onClick={() => handleModalOpen(record.area, index)}
                            >
                                编辑
                            </Button>
                            <Button
                                ...
                                type='link'
                                onClick={() =>
                                    onChange?.(dataSource?.filter((v, i) => i !== index))
                                }
                            >
                                删除
                            </Button>
        ...
        props.flag &&
            columnsData.splice(1, 0, {
                title: <span className={styles.required}>包邮条件</span>,
                render(_va, _re, index: number) {
                    return (
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                            实付满
                            <ProFormDigit
                                name={['expressTemplateAreas', index, 'price']}
                                rules={mmFormRule.required}
                                fieldProps={{
                                    precision: 2,
                                    min: 0,
        ...
        return columnsData;
    }, [isAllArea, valuationType, dataSource, props.flag]);
    ...
    function handleModalOk(selectedTree: ICityTree[]) {
        ...
        // if (selectedTree.some(({ title }) => value.find(({ text }) => text.includes(title)))) {
        //     return message.error('不能添加重复区域');
        // }
