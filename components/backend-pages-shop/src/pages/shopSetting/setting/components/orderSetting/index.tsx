  ...
  return (
    ...
    >
      <Form {...defaultFormProps} form={form}>
        <Form.Item label="订单支付时间" name={order_payment_waiting_time_minutes} rules={mmFormRule.required}>
          <ItemNumber addonAfter="分钟" disabled={disabled} min={1} max={9999} precision={0} extra="*不填则系统默认30分钟" />
        </Form.Item>

        <Form.Item label="发货后自动收货时间" name={order_receiving_waiting_time_day} rules={mmFormRule.required}>
