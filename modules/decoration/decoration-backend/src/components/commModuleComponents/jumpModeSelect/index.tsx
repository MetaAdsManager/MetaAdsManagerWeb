...
import { ImageLinkDataDTO } from '@MetaAdsManager-modules/decoration-data';
            ...
            {value.jumpMode === EJumpLinkMode.Link ? (
                <JumpType
                    ...
                    onChange={(link) =>
                        onChange({
                            ...value,
                            link: { type: link.type, content: JSON.stringify(link.content) }
                        })
                    }
                />
            ) : (
                <HotZoneModal
                    value={value.hotZones || []}
                    img={value.url}
                    onChange={(hotZones) => onChange({ ...value, hotZones })}
                />
            )}
        </div>
    );
