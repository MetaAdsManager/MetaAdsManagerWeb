...
import { EChooseGoodType } from '@MetaAdsManager-modules/decoration-data/src/enums/EChooseGoodType';
import { api } from '@MetaAdsManager/backend-api/src/request';
import { MarketingActivityVo } from '@MetaAdsManager/backend-api/src/request/data-contracts';
...
import { EActivityStatus } from '@MetaAdsManager/shop-data/src/enums/activity/EActivityStatus';
...
const Component: FC<ISettingComponetProps> = (props) => {
    ...
    const { tableProps, selectedValue } = useActivityTableService({
        visible,
...
function useActivityTableService({ visible, data }) {
    ...
    const tableProps: ProTableProps<any, any> = {
        ...
        request: useProTableRequest(
            (params) =>
                api['/admin/activity/full_GET']({
                    ...params,
                    activityStatus: EActivityStatus.Use
                }),
            {
                dataFormat: (data) => {
                    ...
                    return data.map((it) => ({
                        ...it,
                        activityTime: [it.startTime, it.endTime],
                        _isFinish: dayjs().isAfter(dayjs(it.endTime), 'second')
                    }));
                }
            }
        ).request
