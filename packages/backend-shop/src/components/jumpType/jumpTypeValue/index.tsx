...
const Component: FC<IJumpTypeValueProps> = (props) => {
    ...
    const prefix = useMemo(
        () => (jumpTypePrefix ? MJumpType[jumpValue.type] + '-' : ''),
        [jumpTypePrefix, jumpValue]
    );

    const content: any = useMemo(() => {
        return typeof jumpValue.content === 'string'
            ? JSON.parse(jumpValue.content)
            : jumpValue.content;
    }, [jumpValue.content]);
        ...
        <span className={styles.jumpTypeValueStyle}>
            {prefix +
                ({
                    ...
                    [EJumpType.GoodsGroup]: content.categoryName,
                    [EJumpType.Hotel]: content.name
                }[jumpValue.type] || '')}
        </span>
    );
};
