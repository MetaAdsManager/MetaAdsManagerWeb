...
import { EOrderStatus, MOrderStatus } from '@MetaAdsManager/shop-data/src/enums/order/EOrderStatus';
...
const typeMaps: any[] = [
    ...
    [EOrderStatus.UNSHIPPED, [ORDER_STATUS_UNSHIPPED]],
    ...
    [
        EOrderStatus.CANCEL,
        [ORDER_STATUS_SYS_CANCEL, ORDER_STATUS_USER_CANCEL, ORDER_STATUS_REFUND_CANCEL]
    ],
    [EOrderStatus.GROUPING, [ORDER_STATUS_GROUP]],
    [EOrderStatus.ERP, [ORDER_STATUS_ERP]],
    [EOrderStatus.ZT, [301]],
    [EOrderStatus.PART_SHIPPED, [ORDER_STATUS_PART_SHIPPED]]
];
...
export default function useOrderTypes(type?: EWholeOrderStatus) {
    ...
    const orderText = useMemo(
        () => (orderState === undefined ? '' : MOrderStatus[orderState]),
        [orderState]
    );
