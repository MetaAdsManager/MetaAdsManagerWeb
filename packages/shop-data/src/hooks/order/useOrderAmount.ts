...
import { EActivityType } from '../../enums/activity/EActivityType';
...
export default function useOrderAmount(data: OrderVO) {
    ...
    } = useMemo(() => countItemDeductionAmount(data), [data]);
    ...
    return {
         ...
         */
        payAmount: [
            EWholeOrderStatus.ORDER_STATUS_PENDING_PAYMENT,
            EWholeOrderStatus.ORDER_STATUS_PAY_ERROR
...
export function calcScoreDeductionAmount(data: OrderVO) {
    ...
    return items.reduce(
        (total, { itemsPointDeduction = 0 }) => mmAdds(total, itemsPointDeduction),
        0
    );
}
...
export function calcCouponDeductionAmount(data: OrderVO) {
    ...
    return items.reduce(
        (total, { itemsCouponDeduction = 0 }) => mmAdds(total, itemsCouponDeduction),
        0
    );
}
...
export function countItemDeductionAmount(data: OrderVO) {
    const { items = [], orderMasterMarketingList = [] } = data;
    ...
    const couponDeductionAmount = orderMasterMarketingList.reduce((total, current) => {
        if (current.marketingType === EActivityType.Coupon) {
            return mmAdds(total, current.discountAmount || 0);
        }
        return total;
    }, 0);
    ...
    return items.reduce(
        (
            result,
            {
                itemsCouponDeduction = 0,
                itemsActiveDeduction = 0,
                itemsPointDeduction = 0,
                itemsUserDeduction = 0,
                itemsShippingDeduction = 0
            }
        ) => ({
            ...
            freeShippingDeductionAmount: mmAdds(
                result.freeShippingDeductionAmount,
                itemsShippingDeduction
            )
        }),
        deduction
    );
