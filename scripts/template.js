 ...
 */
const templateDir = {
  basic: ['@MetaAdsManager/backend-template', '@MetaAdsManager/taro-template'],
  shop: ['@MetaAdsManager/backend-shop', '@MetaAdsManager/taro-shop-component'],
  'shop-waimai': ['@MetaAdsManager/backend-shop-waimai', '@MetaAdsManager/taro-shop-waimai'],
}[scope]
...
// 去重并合并出最终需要保留的包
// 例如 ["@MetaAdsManager-modules/decoration-backend",...]
const allValidPacks = [...new Set(recursionDependencis(templateDir))].concat(templateDir).concat(['eslint-config-wmeimob', 'umi-plugin-access-code'])
...
function recursionDependencis(packages) {
  return packages.reduce((result, package) => {
    ...
    const scopeNameReg = /^@MetaAdsManager\/|^@MetaAdsManager-modules\//
